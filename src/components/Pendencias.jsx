    ...
    <CardShell title="Pendências" icon="⚠️" actionLabel="Ver todas →" onAction={onAction}>
      {items.length === 0 ? (
        <div className="px-5 py-10 text-center">
          ...
          <p className="mt-2 text-sm text-[#8B7BAD]">Nenhuma pendência em aberto!</p>
        </div>
      ) : (
        <div className="divide-y divide-[#F0E6F6]">
          {items.map((item) => (
            <article key={item.name + item.description} className="flex items-center gap-3 px-5 py-4 transition hover:bg-[#FFF8FB]">
